...
- **v156（2026-08-14，全量回归 261/0 PASS）**：深层语法审计修正——① `x is T` 是编译期类型兼容检查（builder.rs __as__/is 分支，ir_types_compatible 生成 Bool 字面量，零运行时 RTTI），非违规；② 文档对齐：00-词法基础 浮点下划线 3.14_15 已支持、04-表达式 is 增补编译期类型检查用法；③ __cast__/__try_cast__ 双缺检查落地：`x as T` 自定义类型（struct_methods 已登记）无两魔法方法 → LZ 编译期报错（is_builtin_cast 放行基本数值互转 + 内置容器基名 List/Vec/Dict/Option/Result/Box/Tuple/String/Any 标注，AST 中 List<int> 解析为整串 "Vec<i64>" 需按 `<` 前基名匹配）
