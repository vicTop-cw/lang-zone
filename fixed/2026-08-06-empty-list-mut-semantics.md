...
### IR 路线复验结果
- `DEMO/03_variables/mutable_let.lz` 新增 `let mut t = "Hi"; t += "!"; print(t)` 经 `lang-zone <path> --emit=ir` 验证：EXIT=0，IR 正确 emit：
  ```
  let mut t: str = [str] "Hi"
  [str] t = [?] binop [str] t + [str] "!"
  ```
  -> 证明 IR builder / parser 已正确设置 `is_mut`，`let mut` 降级为可变绑定，语义对齐 Rust。问题 2 在 IR 层已修复。
- `DEMO/06_control_flow/while_let.lz` 已改为 `let mut result: List<int> = []`，IR emit 通过（空列表需注解语义已对齐）。问题 1 在 IR 层已对齐。
- `cargo test --test mod`：51 passed；`cargo test --test ir_snapshots`：50 passed。
- DEMO 全量 `--emit=ir` 192 文件：160 OK / 32 FAIL（32 全部命中 known_demo_ir_failures，无新失败、无回归）。

### 结论
IR 路线层面，两个语义问题均已对齐/修复，无 P0-P3 未解决 Bug。
（注：完整 type-checker / borrow-checker 验证超出 IR 路线范围，由 lz-3 开发周期负责；IR 降级已正确保留 mutability 语义。）
