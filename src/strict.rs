...
fn collect_unsafe_fns(module: &Module) -> std::collections::HashSet<String> {
    module
        .functions
        .iter()
        .filter(|f| f.decorators.iter().any(|d| d.name == "unsafe"))
        .map(|f| f.name.clone())
        .collect()
}
        ...
        match s {
            ...
            Stmt::While { cond, body, .. } => {
                visit_expr(cond, &mut |x: &Expr| {
                    if let Expr::MethodCall { method, .. } = x {
                        if method == "unwrap" {
                            v.push(viol("S002", format!("fn `{}`: .unwrap()", fn_name), None));
                        }
                    }
                });
