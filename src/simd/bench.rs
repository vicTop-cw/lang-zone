...
use crate::simd::{DType, Simd, SimdOps, SimdStack};
...
fn bench_reduce_sum() {
    ...
    println!(
        "[reduce_sum] SIMD: {:>8}ns | plain: {:>8}ns | {:.2}x",
        r.simd_ns,
        r.plain_ns,
        r.speedup()
    );
...
fn bench_filter() {
    ...
    println!(
        "[filter]     SIMD: {:>8}ns | plain: {:>8}ns | {:.2}x",
        r.simd_ns,
        r.plain_ns,
        r.speedup()
    );
...
fn bench_cumsum() {
    ...
    println!(
        "[cumsum]     SIMD: {:>8}ns | plain: {:>8}ns | {:.2}x",
        r.simd_ns,
        r.plain_ns,
        r.speedup()
    );
...
fn bench_mean_std() {
    ...
    println!(
        "[mean+std]   SIMD: {:>8}ns | plain: {:>8}ns | {:.2}x",
        r.simd_ns,
        r.plain_ns,
        r.speedup()
    );
...
fn bench_summary() {
    println!(
        "\n═══ SIMD vs Plain Rust — Benchmark Summary ({} elements, {} iters) ═══",
        N, ITERS
    );
