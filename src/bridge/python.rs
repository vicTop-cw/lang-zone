...
pub struct PyO3Bridge {
    ...
    /// 正向桥接：lz 调用 Python 模块（对标 Mojo `from python import numpy`）
    /// 模块名 → 可调用函数名列表
    python_modules: HashMap<String, Vec<String>>,
}

impl PyO3Bridge {
    /// 注册一个可正向调用的 Python 模块
    ///
    /// 示例：`bridge.register_python_module("numpy", vec!["array", "zeros"])`
    /// 之后 lz 中 `import python::numpy` / `from python import numpy` 会被解析为
    /// PyO3 `Python::import_module("numpy")` 惰性加载，`array(...)` 经
    /// `__py_call_numpy_array` 包装调用。
    pub fn register_python_module(&mut self, module: &str, callables: Vec<String>) {
        self.python_modules.insert(module.to_string(), callables);
    }
...
impl Bridge for PyO3Bridge {
    ...
    fn resolve_import_full(&self, module_path: &[String], items: &[String]) -> Option<ImportResolveResult> {
        ...
        Some(ImportResolveResult {
            rust_path: format!("__py_mod_{}", module),
            type_aliases: vec![("PyAny".into(), "pyo3::PyAny".into())],
            requires_shim: true,
            is_tier2: false,
            feature_flags: vec!["py-bridge".into()],
            extern_crates: vec!["pyo3".into()],
            error: None,
        })
    }

    /// 正向调用：`numpy_array(...)` → `__py_call_numpy_array`
    ///
    /// 优先匹配正向桥接注册的 Python 模块函数；其次匹配反向导出（`__py_<name>`）。
    fn resolve_call_full(&self, func_name: &str, _args: &[String]) -> Option<CallResolveResult> {
        for (mod_name, callables) in &self.python_modules {
            if callables.iter().any(|c| c == func_name) {
                return Some(CallResolveResult {
                    rust_path: format!("__py_call_{}_{}", mod_name, func_name),
                    shim: String::new(),
                    module_name: "pyo3".into(),
                    is_macro: false,
                    is_template: false,
    ...
    fn required_shims(&self) -> Vec<String> {
        ...
        for (mod_name, callables) in &self.python_modules {
            shims.push(format!("__py_mod_{}", mod_name));
            for c in callables {
                shims.push(format!("__py_call_{}_{}", mod_name, c));
            }
        }
        shims
    }

    /// 生成正向桥接 shim 代码（惰性模块加载 + 调用包装）
    fn gen_import(&self, module_path: &[String], _items: &[String]) -> String {
        if module_path.first().map(|s| s.as_str()) != Some("python") {
            return String::new();
        }
        let mut out = String::new();
        out.push_str("// Generated by Lang-Zong PyO3Bridge (forward bridge)\n");
        out.push_str("// 对标 Mojo: from python import <module>\n\n");
        for (mod_name, callables) in &self.python_modules {
            out.push_str(&format!(
                "/// 惰性加载 Python 模块 `{}`（PyO3 Python::import_module）\n",
                mod_name
            ));
            out.push_str(&format!(
                "fn __py_mod_{}() -> &'static pyo3::Py<pyo3::PyModule> {{\n",
                mod_name
            ));
            out.push_str("    static MOD: std::sync::OnceLock<pyo3::Py<pyo3::PyModule>> = std::sync::OnceLock::new();\n");
            out.push_str(&format!(
                "    MOD.get_or_init(|| pyo3::Python::with_gil(|py| pyo3::PyModule::import(py, \"{}\").expect(\"import python module {}\")))\n",
                mod_name, mod_name
            ));
            out.push_str("}\n\n");
            for callable in callables {
                ...
                out.push_str("    pyo3::Python::with_gil(|py| {\n");
                out.push_str(&format!(
                    "        let m = __py_mod_{}().bind(py);\n",
                    mod_name
                ));
                out.push_str(&format!(
                    "        m.getattr(\"{}\").expect(\"attr {}\").call1(args).expect(\"call {}\").into()\n",
...
"#;
    ...
    #[test]
    fn test_pyo3_forward_import_and_call() {
        let dir = TempDir::new("lz-pyo3-fwd").unwrap();
        let path = create_test_manifest(&dir);
        let mut bridge = PyO3Bridge::load(&path).unwrap();
        ...
        assert_eq!(rev.rust_path, "__py_add");
    }

    #[test]
    fn test_pyo3_forward_shims_generation() {
        let dir = TempDir::new("lz-pyo3-shims").unwrap();
        let path = create_test_manifest(&dir);
        let mut bridge = PyO3Bridge::load(&path).unwrap();
        bridge.register_python_module("numpy", vec!["array".into()]);

        let shims = bridge.required_shims();
        assert!(shims.contains(&"__py_mod_numpy".to_string()));
        assert!(shims.contains(&"__py_call_numpy_array".to_string()));

        let code = bridge.gen_import(&["python".into(), "numpy".into()], &[]);
        assert!(code.contains("__py_mod_numpy"));
        assert!(code.contains("PyModule::import(py, \"numpy\")"));
        assert!(code.contains("__py_call_numpy_array"));

        // 非 python 命名空间生成空
        assert_eq!(bridge.gen_import(&["std".into(), "io".into()], &[]), "");
    }
