...
CATALOG = [
    ...
    # K01: raise 基本代码生成
    dict(id="K01", title="raise 基本代码生成", category="raises", priority="P1", mode="rust",
         source='enum E =\n  A(str)\ndef f() -> int raises E =\n  raise E.A("boom")\n  0\n',
         present=["Result<i64, E>", "return Err(E::A", "std::fmt::Display for E",
                  "std::error::Error for E"], absent=[],
         note="raise 生成 return Err; raises 包装 Result 返回; 异常枚举自动派生 Display/Error。"),

    # K02: raises 无显式返回类型 → Result<(), E>
    dict(id="K02", title="raises 无返回类型", category="raises", priority="P1", mode="rust",
         source='enum E =\n  Bad(str)\ndef g() raises E =\n  raise E.Bad("fail")\n',
         present=["Result<(), E>", "return Err(E::Bad"], absent=[],
         note="raises 但无 -> 标注时默认包 Result<(), E>。"),

    # K03: raise enumm 无参变体
    dict(id="K03", title="raise 无参变体", category="raises", priority="P1", mode="rust",
         source='enum E =\n  A\ndef f() -> int raises E =\n  raise E.A\n  0\n',
         present=["return Err(E::A)"], absent=[],
         note="无参枚举变体 raise 生成 return Err(E::A)。"),

    # K04: 无 raise 的函数也能有 raises
    dict(id="K04", title="无 raise 的 raises 函数", category="raises", priority="P1", mode="rust",
         source='enum E =\n  Bad(str)\ndef ok() -> int raises E =\n  42\n',
         present=["Result<i64, E>", "Ok(42)", "std::fmt::Display for E"], absent=[],
         note="即使无 raise，raises 标注也正确生成 Result 返回类型。"),

    # K05: struct 不受 raises 影响
    dict(id="K05", title="struct 不受 raises 影响", category="raises", priority="P1", mode="rust",
         source='struct Point =\n  x: f64\n  y: f64\nenum E =\n  Bad(str)\ndef f() -> Point raises E =\n  raise E.Bad("err")\n  Point(x: 1.0, y: 2.0)\n',
         present=["Result<Point, E>", "struct Point", "#[derive(Clone)]"], absent=["std::fmt::Display for Point"],
         note="struct 不因 raises 获得 Display/Error。"),

    # === async/await (L01-L04) ===

    # L01: await 基本代码生成
    dict(id="L01", title="await 基本代码生成", category="async_await", priority="P1", mode="rust",
         source='async def f() -> int =\n  42\nasync def g() -> int =\n  v = await f()\n  v\n',
         present=["async fn f()", "async fn g()", "{ f() }.await"], absent=[],
         note="await expr 生成 { expr }.await。"),

    # L02: await 链式调用
    dict(id="L02", title="await 链式调用", category="async_await", priority="P1", mode="rust",
         source='async def a() -> int = 1\nasync def b() -> int = await a() + await a()\n',
         present=["async fn", "{ a() }.await"], absent=[],
         note="多个 await 在同一表达式中正确展开。"),

    # L03: async + raises 组合
    dict(id="L03", title="async + raises 组合", category="async_await", priority="P1", mode="rust",
         source='enum E =\n  Bad(str)\nasync def f() -> int raises E =\n  await g()\nasync def g() -> int raises E =\n  1\n',
         present=["async fn f() -> Result<i64, E>", "async fn g() -> Result<i64, E>",
                  "{ g() }.await", "std::fmt::Display for E"], absent=[],
         note="async + raises 共存，await 正确生成。"),

    # L04: await 将结果赋给变量
    dict(id="L04", title="await 赋值", category="async_await", priority="P1", mode="rust",
         source='async def fetch() -> int =\n  data = await get()\n  data * 2\nasync def get() -> int =\n  42\n',
         present=["async fn fetch()", "{ get() }.await", "data * 2"], absent=[],
         note="await 结果正确赋给变量。"),

    # === test framework (M01-M04) ===
    ...
    dict(id="M02", title="suite 测试套件", category="test_framework", priority="P1", mode="rust",
         source='def g() -> int = 1\nsuite "group":\n  test a:\n    assert g() == 1\n  test b:\n    assert g() != 2\n',
         present=["mod group", "#[test]", "fn a()", "fn b()"], absent=[],
         note="suite 生成 mod，内嵌 test 生成 #[test] fn。"),

    # M03: 无引号自由文本测试名
    dict(id="M03", title="无引号测试名", category="test_framework", priority="P1", mode="rust",
         source='def h() -> int = 0\ntest simple name here:\n  assert h() == 0\n',
         present=["fn simple_name_here()"], absent=[],
         note="无引号自由文本测试名转 snake_case 函数名。"),
