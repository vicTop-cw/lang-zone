...
# -*- coding: utf-8 -*-
"""
Lang-Zong 编译器测试驱动 (Test Harness)
=======================================
所有测试产物仅落在本次测试运行目录 (test-suite/{yyyyMMdd}-NN/) 内：
  - 输入 .lz 与生成 .rs 临时文件 -> ./_work/
...
判定规则
--------
- tokens / ast 模式: 断言 stdout 含 present 子串、不含 absent 子串, 退出码==0
- rust   模式     : 断言生成的 <id>.rs 含 present 子串、不含 absent 子串, 退出码==0
- error  模式     : 断言退出码==1 (且 !=101, 非 panic), stderr/stdout 含 present 子串
- 任一模式退出码==101 -> 判定 CRASH (严重缺陷)
"""
...
CATALOG = [
    ...
    dict(id="F12", title="安全导航 + 空合并", category="functional", priority="P1", mode="rust",
         ...
         present=[".map(|x| x.name)", "unwrap_or"],
         absent=[],
    ...
    dict(id="F13", title="guard let 模式守卫", category="functional", priority="P1", mode="rust",
         ...
         present=['let Some(v) = opt else {', 'return "none".to_string();'],
         absent=[],
    ...
    dict(id="B01", title="空输入", category="boundary", priority="P1", mode="tokens",
         ...
         present=["Eof"],
         absent=["Def"],
         note="空源应得到仅含 Eof 的 Token 流, 不崩溃。"),
    ...
    dict(id="G03", title="生成器构建块 *:", category="buildblock", priority="P1", mode="rust",
         source="def f() =\n    x = make *:\n        yield (1,)\n        yield (2,)",
         present=["let mut __bb: Vec<__Pack>", "IterStopException"],
         absent=[],
         note="*: 语法为 `x = callee *: 块`; 收集参数包向量并以 IterStopException 收尾。"),
    ...
    dict(id="G07", title="调用块返回值非法(应拒绝)", category="buildblock", priority="P1", mode="error",
         ...
         present=["返回值必须是元组"],
         absent=[],
         note="调用构建块返回值须满足 BuildParams 约束 (元组/字典/结构体/标识符)。"),
    ...
    dict(id="E03", title="表达式非法 token", category="exception", priority="P1", mode="error",
         source="def f() = @@@",
         present=["Unexpected token"],
         absent=[],
         note="非法 token 在表达式位置应报错, 不 panic。"),
    ...
    dict(id="E06", title="浮点溢出静默归零", category="exception", priority="P1", mode="tokens",
         source="x = 1e400",
         present=["FloatLit(inf)"],
         absent=["LexError"],
         note="超出 f64 范围 -> 解析为 +inf 静默通过 (潜在缺陷: 应报错而非静默 inf/NaN)。"),
]
...
def run_case(case):
    ...
    rs_text = ""
    if mode == "rust" and os.path.exists(rs_path):
