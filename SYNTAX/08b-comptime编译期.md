...
| 语法 | 原因 |
|------|------|
| `match` 表达式 | 编译期用 if/elif/else |
| 构建块 `=:`/`~:`/`*:` | 编译期不支持 |
| 路径访问 `a.b.c` | 编译期不支持（模块路径解析受限） |
| 运行时函数调用 | 函数内部依赖运行时状态，不可在编译期执行 |
| 访问运行时变量 | 编译期不存在 |
| 闭包 | 编译期不支持 |
| 列表推导 | 编译期不支持 |

> **注意**：方法调用（如 `list.push()`、`config.contains()`）和索引（如 `arr[0]`、`dict["key"]`）**在 comptime 块内创建的局部变量上可用**。

### 4.3 comptime def — 编译期函数
...
与普通 `def` 的区别：
- `comptime def` 只能在 `comptime:` 块或 `const` 初始化表达式中调用（`const` 上下文隐式为编译期，无需再写 `comptime` 关键字）
- 编译期函数的参数同样只能使用编译期可知的值（`const`、字面量等）
- 运行时代码中调用 `comptime def` 会触发编译错误

> **相关：形参位 `comptime`**。若只需约束**个别形参**、函数本身仍可在运行时调用，用形参修饰 `def f(comptime n: int)`，见 [03b-函数泛型](03b-函数泛型.md) §五。两者区别：`comptime def` 是**整函数**编译期、运行时不可调用；形参 `comptime` 只约束该形参、函数仍是普通运行时函数。
