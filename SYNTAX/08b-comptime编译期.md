...
| 语法 | 原因 |
|------|------|
| `match` 表达式 | 编译期暂不支持，请使用 if/elif/else |
| 构建块 `=:`/`~:`/`*:` | 编译期不支持 |
...
> **注意**：方法调用（如 `list.push()`、`config.contains()`）和索引（如 `arr[0]`、`dict["key"]`）**在 comptime 块内创建的局部变量上可用**。以上限制部分将在自举后进一步放宽（JIT 编译执行模型可调用任意已转译的 Rust 函数）。
